...
# 根据某个字段获取记录
    ...
    static function instance($pConfig = 'default')
    {
        if (empty(self::$instance[$pConfig])) {
            $tDB = Yaf_Application::app()->getConfig()->get("database")['mysql'];
            self::$instance[$pConfig] = @new PDO($tDB['dsn'], $tDB['user'], $tDB['password'], array(PDO::MYSQL_ATTR_INIT_COMMAND => "SET NAMES utf8"));
        }
